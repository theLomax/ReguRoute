			...
			schema: {
				body: {
					...
					properties: {
						...
						platform: { type: ['string', 'null'], enum: platformEnum },
						ammunition_capacity: { type: 'number', minimum: 1, maximum: 999 },
						nfa_subtype: { type: ['string', 'null'], enum: nfaSubtypeEnum },
						barrel_length_inches: { type: 'number', minimum: 0 },
						overall_length_inches: { type: 'number', minimum: 0 },
						notes: { type: 'string' },
					},
				},
			},
			...
			schema: {
				...
				body: {
					...
					properties: {
						name: { type: 'string', minLength: 1, maxLength: 255 },
						category: { type: 'string', enum: categoryEnum },
						accepts_detachable_magazine: { type: 'boolean' },
						calibers: { type: 'array', items: { type: 'string' } },
						platform: { type: ['string', 'null'], enum: platformEnum },
						ammunition_capacity: { type: 'number', minimum: 1, maximum: 999 },
						nfa_subtype: { type: ['string', 'null'], enum: nfaSubtypeEnum },
